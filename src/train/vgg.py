"""VGG model definition, adapted from https://pytorch.org/docs/stable/torchvision/models.html, 
to take single channel instead of 3-channel images.

Also added the option of dropout layers 
after the pooling layers for extra 
regularisation.
"""
...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=4, init_weights=True, dropout=False):
...
def make_layers(cfg, batch_norm=False, dropout=False):
    layers = []
    in_channels = 1 # changed from 3 to 1 because grayscale
    for v in cfg:
        if v == 'M':
...
def _vgg(arch, cfg, batch_norm, dropout=False, **kwargs):
    # if pretrained:
